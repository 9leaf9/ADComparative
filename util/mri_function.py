...
# extract all nii files (MRI images) from unzipped folders.
...
# %%
# filter out the repeatative scans collected from the same person on the same day
...
By checking, it is found that if the last dimension s[2] 
...
from ADNI(can be seen from the name ofthe file)."""
...
# %%
# functions for preprocessing of augmented data
...
# create a preprocessing procedure for augmented data
...
# %%
# compute the distribution of class labels in a given folder of images
...
# %%
# do random check with the preprocessed images
def check(path):
    names = listdir(path)
