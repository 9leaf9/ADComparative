...
class FirstNet(nn.Module):
    ...
    def __init__(self, f=8):
        ...
        self.fc2 = nn.Linear(32 * 8, 128)
        self.fc3 = nn.Linear(128, 64)
        self.fc4 = nn.Linear(64, 2)
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))

    def forward(self, x1, x2):
        x1 = x1.view((x1.shape[0] * 27, 1, 50, 50, 50))
        x1 = self.layer1(x1)
        x1 = self.layer2(x1)
        # print(x1.shape)
        att_x1 = x1.view(int(x1.shape[0] / 27), 27, x1.shape[1], -1)
        att_x1 = x1.view(att_x1.shape[0], 27, -1)
        x1_corr1 = torch.bmm(att_x1, att_x1.transpose(1, 2))
        x1_corr1 = torch.softmax(x1_corr1, 2)
        att_x1 = torch.bmm(x1_corr1, att_x1)
        att_x1 = att_x1.view(att_x1.shape[0], att_x1.shape[1], 128, 9, 9, 9)
        att_x1 = att_x1.view(att_x1.shape[0] * 27, 128, 9, 9, 9)
        x1 = att_x1
        x1 = self.layer3(x1)
        # x1 = self.layer4(x1)
        ...
        x1 = self.avgpool(x1)
        # print(x1.shape)
        x1 = x1.view(int(x1.shape[0] / 27), 27, -1)
        x1 = torch.mean(x1, 1)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            # result.to_csv("/data1/qiaohezhe/MRI_Score/log/mci_nc/c3d_patch2.csv", mode='w', index=False,
            #               header=False)
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc/c3d_patch2.csv", mode='w', index=False,
                          header=False)

            early_stopping(val_loss, model)

            if early_stopping.early_stop:
                # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/c3d_patch2/mri_patch.pth")
                print("Early stopping")
                break
    #         torch.save(model,
    #                    "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/c3d_patch2/mri_patch{}.pth".format(epoch))
    # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/c3d_patch2/mri_patch.pth")
