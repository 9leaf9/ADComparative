...
os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, [5, 6, 7, 8]))
start = datetime.now()
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/mci_nc/c3d_patch.csv", mode='w', index=False,
                          header=False)

            early_stopping(val_loss, model)

            if early_stopping.early_stop:
                torch.save(model, "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/c3d_patch/mri_patch.pth")
                print("Early stopping")
                break
            torch.save(model,
                       "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/c3d_patch/mri_patch{}.pth".format(epoch))
    torch.save(model, "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/c3d_patch/mri_patch.pth")
