    ...
    for epoch in range(epochs):
        ...
        for i, data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            loss_contrastive = torch.mean((1 - labels3.float()) * torch.pow(euclidean_distance.float(), 2) +
                ...
                torch.clamp(5.0 - euclidean_distance.float(), min=0.0), 2))
            ...
            loss = 0.8 * loss_group + 0 * loss_compare + 0.2 * loss_contrastive + 0 * loss_score
            print("loss", loss)
        ...
        with torch.no_grad():
            ...
            result_list.append(
                [epoch, round(train_loss, 3), round(val_loss, 3), round(correct1 / total1, 3),
                 ...
                 round(train_group_loss, 3),  round(train_compare_loss, 3), round(train_score_loss, 3), round(train_contrastive_loss, 3),
            ...
            name = ['epoch', 'train_loss', 'val_loss', 'val_acc1', 'val_acc2', 'recall', 'precision', 'F1',
                    'train_group_loss', 'train_score_loss', 'train_compare_loss', 'train_contrastive_loss', 'val_group_loss',
                    'val_compare_loss','val_score_loss',
                    'val_contrastive_loss']

            result = pd.DataFrame(columns=name, data=result_list)

            # result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/paper/joint/c3d_joint7210"
            #               ".csv", mode='w', index=False,
            #               header=False)
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc_all/paper/compare/c3d_joint8020_dis5"
                          ".csv", mode='w', index=False,
                          header=False)
            ...
            if early_stopping.early_stop:
                # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/model/joint/c3d_joint7210/c3d_score.pth")
                # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_all/model/joint/c3d_joint7210/c3d_score.pth")
                print("Early stopping")
                break
    #         torch.save(model,
    #                    "/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/model/joint/c3d_joint7210/c3d_score{}.pth".format(epoch))
    # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/model/joint/c3d_joint7210/c3d_score.pth")
    #         torch.save(model,
    #                    "/data1/qiaohezhe/MRI_Score/log/ad_nc_all/model/joint/c3d_joint7210/c3d_score{}.pth".format(epoch))
    # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_all/model/joint/c3d_joint7210/c3d_score.pth")
