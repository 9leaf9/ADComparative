...
class FirstNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x1_corr1 = torch.bmm(x1_in, x1_in.transpose(1, 2))
        x1_corr1 = torch.mean(x1_corr1, 2)
        x1_corr1 = torch.softmax(x1_corr1, 1)
        x1_corr1 = x1_corr1.view(x1_corr1.shape[0], 1, x1_corr1.shape[1])
        att_x1 = torch.bmm(x1_corr1, x1_in)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/mci_nc/c3d_conv_mean.csv", mode='w',
                          index=False, header=False)

            # result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc/c3d_conv_mean.csv", mode='w',
            #               index=False, header=False)
