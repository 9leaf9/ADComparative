...
import pandas as pd
from datasets.ADNI_score import AdniDataSet
...
if __name__ == "__main__":
    ...
    train_db = AdniDataSet(train_data_path, train_img_path, sets)
    val_db = AdniDataSet(val_data_path, val_img_path, sets)
    """将训练集划分为训练集和验证集"""

    # train_db, val_db = torch.utils.data.random_split(train_data, [700, 180])
    print('train:', len(train_db), 'validation:', len(val_db))

    train_loader = DataLoader(dataset=train_db, batch_size=24, shuffle=True)
    val_loader = DataLoader(dataset=val_db, batch_size=24, shuffle=True)
    ...
    for epoch in range(epochs):
        ...
        for i, data in tqdm(enumerate(train_loader), total=len(train_loader)):
            inputs, mask, labels, score = data
            inputs, mask, labels, score = inputs.to(device), mask.to(device), labels.to(device), score.to(device)
            optimizer.zero_grad()
            logps, score_predict = model.forward(inputs, mask)
            score_predict = torch.sigmoid(score_predict)
            # print(score_predict)
            # print(score)
            loss1 = criterion(logps, labels)
            loss2 = F.mse_loss(score, score_predict)
            # print(loss1)
            # print(loss2)
            loss = torch.add(loss1.float(), loss2)
        ...
        with torch.no_grad():
            print("validation...")
            for inputs, mask, labels, score in val_loader:
                inputs, mask, labels, score = inputs.to(device), mask.to(device), labels.to(device), score.to(device)
                output, score_predict = model(inputs, mask)
                score_predict = torch.sigmoid(score_predict)
                ...
                # print(loss1)
                # print(loss2)
                loss = torch.add(loss1.float()*0.8, loss2*0.2)
                running_loss += loss.item()
                total += labels.size(0)
            ...
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/mci_nc/c3d_score1.csv", mode='w', index=False,
                          header=False)
