...
import pandas as pd
from datasets.ADNI_joint import AdniDataSet_train
from datasets.ADNI_joint import AdniDataSet_val
...
class FirstNet(nn.Module):
    ...
    def __init__(self, f=8):
        ...
        # self.layer7.add_module('conv4',
        #                        nn.Conv3d(in_channels=32 * f, out_channels=64 * f, kernel_size=2, stride=1, padding=1,
        #                                  dilation=2))
        # self.layer7.add_module('bn4', nn.BatchNorm3d(num_features=64 * f))
        # self.layer7.add_module('relu4', nn.ReLU(inplace=True))
        # self.layer7.add_module('max_pooling4', nn.MaxPool3d(kernel_size=5, stride=2))
        #
        ...
        self.fc10 = nn.Linear(512 * 3 * 3 * 3, 256)
    ...
    def forward(self, x1, x2):
        ...
        x_group = x_group1 - x_group2
        x_group = x_group.view(x_group.shape[0], -1)
        x_group = self.fc10(x_group)
        x_g = self.fc11(x_group)
        ...
        #
        # x_s = F.sigmoid(x_score1-x_score2)
        ...
        return x1, x2, x_embedding1, x_embedding2, x_s, x_g,  x_s1, x_s2
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for i, data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            logps1, logps2, embedding1, embedding2, logps_s, logps_g, logps_score1, logps_score2 = model.forward(inputs1,
                                                                                                        inputs2)
            print(logps_s)
            print(logps_score1)
            print(logps_score2)
            ...
            #     torch.clamp(7.0 - euclidean_distance.float(), min=0.0), 2))
            loss_contrastive = criterion(logps_g, labels3)
            ...
            consine_distance3 = F.cosine_similarity(logps_score1, score1.float(), dim=0)
            consine_distance4 = F.cosine_similarity(logps_score2, score2.float(), dim=0)

            loss_score = 0.5 * loss3 + 0.5 * loss4
            print("loss_score", loss_score)
            consine_distance = 0.5 * consine_distance3 + 0.5 * consine_distance4
            print("consine_distance", consine_distance)

            loss = 0.6 * loss_group + 0 * loss_compare + 0.4 * loss_contrastive + 0 * loss_score
            print("loss", loss)
            ...
            # print("train contrastive ground truth", labels3)
            # print("train contrastive predict", euclidean_distance)
            print("train contrastive ground truth", labels3)
            print("train contrastive predict", logps_g)
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(val_loader), total=len(val_loader)):
                ...
                inputs1, inputs2, labels1, labels2, labels3, labels4, score1, score2 = inputs1.to(device), inputs2.to(
                    ...
                    device), labels3.to(device), labels4.to(device), score1.to(device), score2.to(
                    device)
                output1, output2, embedding1, embedding2, logps_s, logps_g, logps_score1, logps_score2 = model.forward(inputs1,
                                                                                                              inputs2)
                ...
                # loss_contrastive = torch.mean((1 - labels3.float()) * torch.pow(euclidean_distance.float(), 2) +
                #                               (labels3.float()) * torch.pow(
                #     torch.clamp(7 - euclidean_distance.float(), min=0.0), 2))
                loss_contrastive = criterion(logps_g, labels3)
                print("loss_contrastive", loss_contrastive)
            ...
            val_contrastive_loss = running_contrastive_loss / len(val_loader)
            ...
            result_list.append(
                [epoch, round(train_loss, 3), round(val_loss, 3), round(correct1 / total1, 3),
                 ...
                 round(train_group_loss, 3), round(train_compare_loss, 3), round(train_score_loss, 3),
                 round(train_contrastive_loss, 3),
                 round(val_group_loss, 3), round(val_compare_loss, 3), round(val_score_loss, 3),
                 round(val_contrastive_loss, 3)])

            # 输入日志
            name = ['epoch', 'train_loss', 'val_loss', 'val_acc1', 'val_acc2', 'recall', 'precision', 'F1', 'AUC',
                    'train_group_loss', 'train_score_loss', 'train_compare_loss', 'train_contrastive_loss',
                    'val_group_loss',
                    'val_compare_loss', 'val_score_loss',
                    'val_contrastive_loss']

            result = pd.DataFrame(columns=name, data=result_list)

            # result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/paper1/joint/c3d_joint8020"
            #               ".csv", mode='w', index=False,
            #               header=False)
            # result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc_all/paper1/joint/c3d_joint8020"
            #               ".csv", mode='w', index=False,
            #               header=True)

            # result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc_all/paper1/ablation/c3d_joint8020_8"
            #               ".csv", mode='w', index=False,
            #               header=True)
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/paper1/ablation/c3d_joint8020_4"
                          ".csv", mode='w', index=False,
                          header=True)
            ...
            if early_stopping.early_stop:
                # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/model1/joint/c3d_joint8020/c3d_score.pth")
                # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_all/model1/joint/c3d_joint8020/c3d_score.pth")
                print("Early stopping")
                break
    #         torch.save(model,
    #                    "/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/model1/joint/c3d_joint8020/c3d_score{}.pth".format(epoch))
    # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_adni2/model1/joint/c3d_joint8020/c3d_score.pth")
    #         torch.save(model,
    #                    "/data1/qiaohezhe/MRI_Score/log/ad_nc_all/model1/joint/c3d_joint8020/c3d_score{}.pth".format(epoch))
    # torch.save(model, "/data1/qiaohezhe/MRI_Score/log/ad_nc_all/model1/joint/c3d_joint8020/c3d_score.pth")
