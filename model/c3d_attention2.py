...
class FirstNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x1_corr1 = torch.softmax(x1_corr1, 2)
        att_x1 = x1
        # att_x1 = torch.squeeze(att_x1)
        att_x1 = att_x1.transpose(1, 2)
        att_x1 = att_x1.contiguous().view(att_x1.shape[0], att_x1.shape[1], -1)
        # print(x1_corr1.shape)
        # print(att_x1.shape)
        att_x1 = torch.bmm(x1_corr1, att_x1)
        att_x1 = att_x1.view(att_x1.shape[0], 1, att_x1.shape[1], 90, 90)

        x1_sw2 = self.sw2(x1.transpose(2, 3))
        x1_sw2 = x1_sw2.transpose(1, 2)
        x1_sw2 = x1_sw2.contiguous().view(x1_sw2.shape[0], x1_sw2.shape[1], -1)
        x1_corr2 = torch.bmm(x1_sw2, x1_sw2.transpose(1, 2))
        x1_corr2 = torch.softmax(x1_corr2, 2)
        att_x2 = x1
        att_x2 = att_x2.transpose(2, 3)
        att_x2 = torch.squeeze(att_x2)
        att_x2 = att_x2.contiguous().view(att_x2.shape[0], att_x2.shape[1], -1)
        att_x2 = torch.bmm(x1_corr2, att_x2)
        att_x2 = att_x2.view(att_x2.shape[0], 1, att_x2.shape[1], 90, 90)

        x1_sw3 = self.sw2(x1.transpose(2, 4))
        x1_sw3 = x1_sw3.transpose(1, 2)
        x1_sw3 = x1_sw3.contiguous().view(x1_sw3.shape[0], x1_sw3.shape[1], -1)
        x1_corr3 = torch.bmm(x1_sw3, x1_sw3.transpose(1, 2))
        x1_corr3 = torch.softmax(x1_corr3, 2)
        att_x3 = x1
        att_x3 = att_x3.transpose(2, 4)
        att_x3 = torch.squeeze(att_x3)
        att_x3 = att_x3.contiguous().view(att_x3.shape[0], att_x3.shape[1], -1)
        att_x3 = torch.bmm(x1_corr3, att_x3)
        att_x3 = att_x3.view(att_x3.shape[0], 1, att_x3.shape[1], 90, 90)

        x1 = x1 + att_x1
        x1 = x1 + att_x2.transpose(2, 3)
        x1 = x1 + att_x3.transpose(2, 4)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            result.to_csv("/data1/qiaohezhe/MRI_Score/log/mci_nc/mri_attention1.csv", mode='w', index=False,
                          header=False)

            early_stopping(val_loss, model)

            if early_stopping.early_stop:
                torch.save(model, "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/mri_attention1/mri_attention.pth")
                print("Early stopping")
                break
            torch.save(model,
                       "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/mri_attention1/mri_attention{}.pth".format(epoch))
    torch.save(model, "/data1/qiaohezhe/MRI_Score/log/mci_nc/model/mri_attention1/mri_attention.pth")
