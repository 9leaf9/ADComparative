...
if __name__ == "__main__":
    file_path = '/data1/qiaohezhe/ADNI/MRI_corr_all/'
    save_path = "/data1/qiaohezhe/ADNI/MRI_augment2/ad_vs_mci/"
    data_path = '/data1/qiaohezhe/ADNI/ADdata/AD_lastest_filter_cn.csv'
    data = pd.read_csv(data_path)
    subject_lsit = data['Subject ID'].tolist()
    group_lsit = data['Group'].tolist()
    random_index = [i for i in range(len(subject_lsit))]
    shuffle(random_index)
    train_index = random_index[: int(len(subject_lsit) * 0.75)]
    ...
    for index in tqdm(train_index):
        ...
        if os.path.exists(mri_path):
            ...
            nb.save(img, save_path + 'train/images/' + "{}.nii".format(str(number)))
            labels.append([number, group_lsit[index]])
            number += 1
            ...
            nb.save(roa_img, save_path + 'train/images/' + "{}.nii".format(str(number)))
            labels.append([number, group_lsit[index]])
            number += 1
            ...
            ela_img_data = elastic_3d(img_data)
            ela_img = nib.Nifti1Image(ela_img_data, np.eye(4))
            nb.save(ela_img, save_path + 'train/images/' + "{}.nii".format(str(number)))
            labels.append([number, group_lsit[index]])
            number += 1

    name = ['index', 'group']
    data_csv = pd.DataFrame(columns=name, data=labels)
    data_csv.to_csv(save_path + "train_label.csv", index=False)
    ...
    for index in tqdm(val_index):
        ...
        if os.path.exists(mri_path):
            '''1.读取  2.增强   3.保存'''
            img = nib.load(mri_path)
            nb.save(img, save_path + 'val/images/' + "{}.nii".format(str(number)))
            labels.append([number, group_lsit[index]])
            number += 1

    name = ['index', 'group']
    data_csv = pd.DataFrame(columns=name, data=labels)
    data_csv.to_csv(save_path + "val_label.csv", index=False)
